...
const getCourseByIdWithReviews = catchAsync(async (req, res) => {
  ...
  const { course, reviews } = await CourseServices.getCourseByIdWithReviews(courseId);
...
const getBestCourse = catchAsync(async (req, res) => {
  ...
  const course = await CourseServices.getCourseById(reviewsWithAverageRating[0]?._id);
