...
const loginUser = async (payload: IAuth) => {
  ...
  const jwtPayload: IJWTPayload = {
    ...
    email: user.email,
    iat: Date.now(),
    exp: Date.now() + (process.env.JWT_ACCESS_EXPIRES_IN as unknown as number) * 1000 * 60 * 60 * 24, // 1 day,
  };
...
const changePassword = async (userData: JwtPayload, payload: IChangePassword) => {
  ...
  if (user.passwordHistory.length > 0) {
    if (
      user.passwordHistory.length >= 1 &&
      (await bcrypt.compare(payload?.newPassword, user.passwordHistory[0]?.password))
    ) {
      return {
        username: false,
        email: user.passwordHistory[0].createdAt,
      };
    } else if (
      user.passwordHistory.length > 1 &&
      (await bcrypt.compare(payload?.newPassword, user.passwordHistory[1]?.password))
    ) {
      return {
        username: false,
        email: user.passwordHistory[1].createdAt,
      };
    }
  }

  // hash new password
  const hashedPassword = await bcrypt.hash(payload?.newPassword, Number(process.env.BCRYPT_SALT_ROUNDS));
