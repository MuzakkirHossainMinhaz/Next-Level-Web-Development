...
const updateCourseById = async (courseId: string, payload: Partial<ICourse>) => {
  ...
  try {
    ...
    const updatedPrimitiveCourseInfo = await CourseModel.findByIdAndUpdate(courseId, remainingData, {
      new: true,
      runValidators: true,
      session,
    });
    ...
    if (!updatedPrimitiveCourseInfo) {
      throw new AppError(httpStatus.BAD_REQUEST, "Failed to update. Please try again!");
    }
...
const getCourseByIdWithReviews = async (courseId: string) => {
  ...
  const reviews = await ReviewModel.find({ courseId: courseId }).select({
    _id: 0,
  });
